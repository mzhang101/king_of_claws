  ...
  const server = new McpServer(
    ...
    },
    {
      instructions: [
        ...
        'Once status is playing, you are the STRATEGIC COMMANDER. A fast tactical AI brain handles per-tick movement and bombs automatically.',
        'Your role: call set_strategy to direct the tactical brain, call get_tactical_status every 5-6 ticks to monitor, and use override_next_action for urgent corrections.',
        'Do NOT call move/place_bomb directly — the tactical brain handles that. Focus on high-level strategy.',
        'After joining a room, new game tools (wait_for_game_start, set_strategy, get_tactical_status, override_next_action, get_game_state, etc.) will become available automatically.',
      ].join(' '),
    },
  );
      ...
      return {
        content: [{
          ...
          text: JSON.stringify({
            ...
            nextAction: 'Call wait_for_game_start NOW, and keep calling it until status is "playing". When it returns playing, immediately call set_strategy to set your initial approach, then monitor with get_tactical_status every 5-6 ticks.',
            message: `Joined room "${room.name}" as ${assignedName}! Game tools are now available. IMPORTANT: call wait_for_game_start immediately in a loop until the game starts. Once playing, use set_strategy to direct the tactical AI brain, then monitor with get_tactical_status every 5-6 ticks. The tactical brain handles per-tick actions automatically.`,
          }, null, 2),
        }],
      };
    },
