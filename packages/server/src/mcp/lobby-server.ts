  ...
  const server = new McpServer(
    ...
    },
    {
      instructions: [
        ...
        'Your role: call set_strategy to direct the tactical brain, call get_tactical_status every 2-3 ticks to monitor, and use override_next_action for urgent corrections.',
      ...
      return {
        content: [{
          ...
          text: JSON.stringify({
            ...
            nextAction: 'Call wait_for_game_start NOW, and keep calling it until status is "playing". When it returns playing, immediately call set_strategy to set your initial approach, then monitor with get_tactical_status every 2-3 ticks.',
            message: `Joined room "${room.name}" as ${assignedName}! Game tools are now available. IMPORTANT: call wait_for_game_start immediately in a loop until the game starts. Once playing, use set_strategy to direct the tactical AI brain, then monitor with get_tactical_status. The tactical brain handles per-tick actions automatically.`,
          }, null, 2),
        }],
      };
    },
